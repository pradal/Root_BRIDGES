...
# Model packages
from root_bridges.root_bridges_growth import Model
# Utility packages
...
def single_run(scenario, outputs_dirpath="outputs", simulation_length=2500, echo=True, log_settings={}):
    root_bridges_growth = Model(time_step=3600, scenario=scenario)

    logger = Logger(model_instance=root_bridges_growth, outputs_dirpath=outputs_dirpath, 
    ...
    try:
        for _ in range(simulation_length):
            ...
            t3 = time.time()
            #root_bridges_growth.run()
            global_performance = pd.concat([global_performance, pd.DataFrame({"time": [root_bridges_growth.time], "log_time": [t2 - t1], "run_time": [t3 - t2], "total_time": [t3 - t1]})])
...
if __name__ == '__main__':
    scenarios = ms.from_table(file_path="inputs/Scenarios_24_06.xlsx", which=["Benchmark_All_Growth"])
